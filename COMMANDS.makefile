...
status: ## Show services status | عرض حالة الخدمات
	@chmod +x start.sh
	@./start.sh status $(ENVIRONMENT)
.PHONY: status

health: ## Check services health | فحص صحة الخدمات
	@echo "$(BLUE)Checking API health...$(NC)"
	@response=$$(curl -fsS http://localhost:5001/health) && echo "$$response" | grep -q '"status"[[:space:]]*:[[:space:]]*"ok"' || echo "$(RED)API is not responding or unhealthy$(NC)"
	@echo ""
...
db-backup: ## Create database backup | عمل نسخة احتياطية
	...
	@docker-compose exec postgres pg_dump -U $(POSTGRES_USER) $(POSTGRES_DB) | gzip > backup-$(shell date +%Y%m%d_%H%M%S).sql.gz
