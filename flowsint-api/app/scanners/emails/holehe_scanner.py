...
class HoleheScanner(Scanner):    
    ...
    def scan(self, email: str) -> Dict[str, Any]:
        try:
            # Puisque scan est synchrone mais holehe est asynchrone,
            # nous devons utiliser asyncio.run
            return asyncio.run(self._perform_holehe_research(email))
        except Exception as e:
            return {"error": f"Unexpected error in Holehe scan: {str(e)}"}
    ...
    async def _perform_holehe_research(self, email: str) -> Dict[str, Any]:
        try:
            ...
            return {"error": f"Error in Holehe research: {str(e)}"}
    
    def postprocess(self, results: Dict[str, Any]) -> Dict[str, Any]:

        results["scanner"] = "holehe"
        if "found" in results and isinstance(results["found"], dict):
            results["platforms"] = list(results["found"].keys())
            ...
            for category, platforms in categories.items():
                results["categories"][category] = [
                    p for p in results["platforms"] if p in platforms
                ]
