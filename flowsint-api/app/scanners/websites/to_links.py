...
class WebsiteToLinks(Scanner):
    ...
    async def scan(self, data: InputType) -> OutputType:
        ...
        for website in data:
            try:
                Logger.info(self.sketch_id, {"message": f"Starting reconspread crawl of {str(website.url)}"})
                ...
                if self.neo4j_conn:
                    self.create_node('website', 'url', str(website.url),
                                   caption=str(website.url), type='website')
                    if main_domain:
                        self.create_node('domain', 'name', main_domain,
                                       caption=main_domain, type='domain')
                        self.create_relationship('website', 'url', str(website.url),
                                               'domain', 'name', main_domain, 'BELONGS_TO_DOMAIN')
                        self.log_graph_message(f"Website {str(website.url)} belongs to domain {main_domain}")
                ...
                def url_handler(url, is_external=False):
                    ...
                    if is_external:
                        ...
                        if domain:
                            ...
                            if self.neo4j_conn:
                                self.create_node('website', 'url', url,
                                               caption=url, type='website')
                                self.create_relationship('website', 'url', str(website.url),
                                                       'website', 'url', url, 'LINKS_TO')
                                self.log_graph_message(f"Website {str(website.url)} links to external website {url}")
                                
                                # Create external domain node and link external website to its domain
                                if domain != main_domain:
                                    self.create_node('domain', 'name', domain,
                                                   caption=domain, type='domain')
                                    self.create_relationship('website', 'url', url,
                                                           'domain', 'name', domain, 'BELONGS_TO_DOMAIN')
                                    self.create_relationship('website', 'url', str(website.url),
                                                           'domain', 'name', domain, 'LINKS_TO_DOMAIN')
                                    self.log_graph_message(f"External website {url} belongs to domain {domain}")
                                    self.log_graph_message(f"Website {str(website.url)} links to external domain {domain}")
                        Logger.info(self.sketch_id, {"message": f"[EXTERNAL] Found: {url} -> Domain: {domain}"})
                    else:
                        internal_urls.append(url)
                        # Create internal website node immediately
                        if self.neo4j_conn and url != str(website.url):  # Don't create duplicate of main website
                            self.create_node('website', 'url', url,
                                           caption=url, type='website')
                            self.create_relationship('website', 'url', str(website.url),
                                                   'website', 'url', url, 'LINKS_TO')
                            self.log_graph_message(f"Website {str(website.url)} links to internal website {url}")
                            
                            # Also link internal websites to main domain
                            if main_domain:
                                self.create_relationship('website', 'url', url,
                                                       'domain', 'name', main_domain, 'BELONGS_TO_DOMAIN')
                        Logger.info(self.sketch_id, {"message": f"[INTERNAL] Found: {url}"})
