...
from app.scanners.websites.to_links import WebsiteToLinks
...
async def test_website_to_links_real_time_neo4j_creation():
    ...
    with patch('app.scanners.websites.to_links.Crawler', MockCrawler):
...
async def test_website_to_links_error_handling_with_neo4j():
    ...
    with patch('app.scanners.websites.to_links.Crawler', mock_crawler_error):
        results = await scanner.scan(websites)
