...
export function MetricsGrid({ investigation }: CaseOverviewPageProps) {
  ...
  const metrics = useMemo(() => [
    ...
    { label: "Entities", value: "247", icon: Users },
    { label: "IOCs", value: "89", icon: Shield },
    { label: "Files", value: "34", icon: Paperclip },
    { label: "Tasks", value: "6/15", icon: CheckCircle },
  ], [sketchCount, analysisCount])

  return (
    <div className="grid border grid-cols-3 md:grid-cols-6 gap-px bg-border rounded-lg overflow-hidden my-6">
      {metrics.map((metric) => (
        <div
          key={metric.label}
