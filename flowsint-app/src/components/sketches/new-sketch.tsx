...
export default function NewSketch({ children, noDropDown = false }: { children: React.ReactNode, noDropDown?: boolean }) {
    ...
    async function handleNewSketch(formData: FormData) {
        formData.append('investigation_id', investigationId as string)
        const result = await sketchService.create(JSON.stringify(formData))
        if (result.success) {
            toast.success("New sketch created.")
            router.navigate({ to: `/dashboard/investigations/${investigationId}/sketches/${result.id}` })
            setOpen(false)
        } else {
