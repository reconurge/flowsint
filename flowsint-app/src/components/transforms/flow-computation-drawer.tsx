...
export function FlowComputationDrawer({
    ...
    const { isLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            const data = await fetch(`http://localhost:5001/transforms/${transformId}/compute`, {
                method: "POST",
                body,
            })
            const json = await data.json()
            setTransformsBranches(json.transformBranches as FlowBranch[])
            setInitialData(json.initialData)
            return data
        },
        enabled: open,
