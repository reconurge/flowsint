...
const ChatHistory = ({ setView, deleteChatMutation, handleCreateNewChat }: { setView: any, deleteChatMutation: any, handleCreateNewChat: any }) => {
    ...
    return (
        <div className='h-full flex flex-col'>
            ...
            {isLoading ? <div className='p-4'><SkeletonList rowCount={4} /></div> :
                chats && chats.length > 0 ? (
                    <div className='grow overflow-auto p-4 flex flex-col gap-2'>
                        {chats.map((chat: Chat) => (<ChatItem switchToChat={switchToChat} deleteChatMutation={deleteChatMutation} confirm={confirm} key={chat.id} chat={chat} />))}
                    </div>
                ) : (
                    <div className="flex-1 flex flex-col items-center justify-center p-8 text-center text-muted-foreground">
                        ...
                        <div className="space-y-2 max-w-sm">
                            ...
                            <p className="text-sm opacity-70">
                                Start chatting with AI to analyze your investigation data and get insights. Your conversation history will appear here.
                            </p>
                        </div>
                        <div className="mt-6">
                            <Button
...
const ChatItem = ({ chat, confirm, deleteChatMutation, switchToChat }: { chat: Chat, confirm: any, deleteChatMutation: any, switchToChat: any }) => {
    const handleDeleteChat = useCallback(async (e: { stopPropagation: () => void; }) => {
        e.stopPropagation()
        if (await confirm({ title: "Are you sure you want to delete this chat?", message: "This action is irreversible." })) {
