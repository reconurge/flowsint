...
const TypeNode = memo(({ data }: TransformNodeProps) => {
    ...
    return (
        <NodeTooltip>
            <NodeTooltipContent isVisible={true} position={Position.Top} className="text-center bg-background text-foreground border shadow-md">
                {data.name}
            </NodeTooltipContent>
            <BaseNode className={cn("shadow-md relative p-0 pl-1.5 bg-background !max-w-[240px]")}
                style={{
                    borderWidth: 2,
