...
export function CreateRelationDialog() {
    ...
    const handleSubmit = async (e: { preventDefault: () => void }) => {
        ...
        const newEdgeObject = { label: relationType, type: "one-way", source: newEdge.source.id, target: newEdge.target.id } as any
        ...
        <Dialog open={openAddRelationDialog} onOpenChange={setOpenAddRelationDialog}>
            <DialogContent className="!w-full !max-w-[600px]">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    {/* Relationship Type */}
                    <div className="space-y-2">
                        <Label htmlFor="relation-type" className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
                            Relationship Type
                        </Label>
                        <Input
                            id="relation-type"
                    ...
                    <DialogFooter>
                        <Button type="button" variant="outline" size="sm" onClick={() => setOpenAddRelationDialog(false)}>
                            Cancel
                        </Button>
...
export function NodeDisplayCard({ node, variant = "default", asRadio = false, radioValue, id }: NodeDisplayCardProps) {
    ...
    if (asRadio) {
        return (
            <label htmlFor={id} className="flex items-center gap-2 px-3 py-2 rounded-md border bg-muted/30 hover:bg-muted/50 transition-colors cursor-pointer data-[state=checked]:border-primary data-[state=checked]:bg-primary/5">
