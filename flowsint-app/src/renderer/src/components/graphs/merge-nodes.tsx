...
export function MergeDialog() {
    ...
    useEffect(() => {
        ...
        const old = {
            ...otherEntitiesToMerge.reduce((acc, entity) => ({
                ...acc,
                ...entity,
            }), {} as GraphNode)
        }
                ...
                <form onSubmit={handleSubmit} className="space-y-4 w-full overflow-hidden">
                    ...
                    <div className="space-y-2 w-full">
                        <Alert className="">
                            ...
                            <AlertDescription>
                                When merging, you can choose which entity is prioritary, simply click it in the list below.
                                Also keep in mind:
                                <div>
                                    <ul className="ml-2 !list-disc">
                                        <li>All <span className="font-semibold">attributes</span> will be merged, and the matching ones values will be based on the prioritary entity</li>
                                        <li>Only one node will be kept, and all <span className="font-semibold">relationships</span> will be merged</li>
                                    </ul>
                                </div>
                            </AlertDescription>
                        </Alert>
                    ...
                    {preview && old && <div className="space-y-2 text-sm">
                    ...
                    <DialogFooter>
                        <Button type="button" variant="outline" size="sm" onClick={() => setOpenMergeDialog(false)}>
                            Cancel
                        </Button>
...
function KeyValueDisplayDiff({ oldRecord, newRecord }: KeyValueDisplayDiffProps) {
    ...
    return (
        <div className={"w-full border-collapse divide-y divide-border"}>
            {diffObject && Object.entries(diffObject)
                ...
                .map(([key, value], index) => {
                    ...
                    return (
                        <div
                            key={index}
                            className={cn("flex w-full bg-card items-center divide-x divide-border p-0",
                                hasChanged && "bg-orange-300/10",
                                isNewKey && "bg-green-400/10",
                                isOldKey && "bg-red-400/10",
                            )}
                        >
                            <div className={cn("w-1/2 px-4 p-2 text-sm text-muted-foreground flex items-center gap-1 font-normal truncate")}>
                            ...
                            <div className="w-1/2 px-4 p-2 text-sm font-medium flex items-center justify-between min-w-0">
                                {!hasChanged ? <div className="truncate font-normal">{JSON.stringify(value.value ?? "")}</div> :
                                    <div className="truncate flex flex-wrap gap-2 font-semibold">
                                        <span className="line-through text-red-300">{JSON.stringify(value.oldValue)}</span><span className={cn(isOldKey ? "text-red-500" : "text-green-500")}>{JSON.stringify(value.newValue ?? "")}</span></div>}
                            </div>
                        </div>
                    )
                })}
