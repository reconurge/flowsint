        ...
        >
            {showButton && (
                ...
                >
                    ...
                    {hasConnections && (
                        <div className="absolute inset-0 -m-4 pointer-events-auto" />
                    )}

                    {!hasConnections ?
                        <>
                            <div
                                className={`bg-gray-300 ${vertical ? "h-16 w-[1px]" : "h-[1px] w-16"}`}
                            />
                    ...
                    {label && <span className="absolute max-w-15 bg-background z-1 truncate text-ellipsis top-1/2 -translate-y-1/2 left-1 text-xs text-muted-foreground">{label}</span>}
                </div>
            )}
        </BaseHandle>
    );
