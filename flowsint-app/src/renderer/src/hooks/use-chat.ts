...
export const useChat = ({ onContentUpdate, onSuccess, editor }: UseChatOptions) => {
    ...
    const aiCompletionMutation = useMutation({
        mutationFn: async ({ chatId, prompt }: { chatId: string, prompt: string }) => {
            setIsAiLoading(true)
            try {
                return await chatService.streamChat(chatId, prompt, (content) => {
    ...
    const handleCustomPrompt = async () => {
        ...
        if (!currentChatId) {
            const chatTitle = selectedNodes.length > 0
                ? `${selectedNodes.map(node => node.data.label).join(", ").slice(0, 100)}`
                : "New Chat"
            ...
            addMessage({ content: customPrompt.trim(), context: selectedNodes.map(node => node.data.label), is_bot: false, id: new Date().getTime().toString(), created_at: new Date().toISOString(), chatId: currentChatId })
    ...
    const createNewChat = async (title?: string) => {
        const chatTitle = title || (selectedNodes.length > 0
