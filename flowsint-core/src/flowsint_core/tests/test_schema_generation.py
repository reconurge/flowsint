...
class TestSchemaGeneration:
    ...
    def test_website_to_domain_scanner_schemas(self):
        ...
        assert (
            input_schema["type"] == "Website"
        ), f"Expected 'Website', got '{input_schema['type']}'"

        # Test output schema generation
        output_schema = scanner.output_schema()
        assert (
            output_schema["type"] == "Domain"
        ), f"Expected 'Domain', got '{output_schema['type']}'"
    ...
    def test_website_to_webtrackers_scanner_schemas(self):
        ...
        output_schema = scanner.output_schema()
        assert (
            output_schema["type"] == "WebTracker"
        ), f"Expected 'WebTracker', got '{output_schema['type']}'"
    ...
    def test_domain_to_website_scanner_schemas(self):
        ...
        input_schema = scanner.input_schema()
        assert (
            input_schema["type"] == "Domain"
        ), f"Expected 'Domain', got '{input_schema['type']}'"

        # Test output schema generation
        output_schema = scanner.output_schema()
        assert (
            output_schema["type"] == "Website"
        ), f"Expected 'Website', got '{output_schema['type']}'"
