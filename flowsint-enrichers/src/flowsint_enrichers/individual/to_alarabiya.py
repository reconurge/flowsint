"""Al Arabiya news enricher: Individual -> Website mentions (Google News RSS)."""
...
from tools.arabic_media.alarabiya import AlArabiyaTool


REL_LABEL = "MENTIONED_IN_ALARABIYA"


@flowsint_enricher
class IndividualToAlarabiyaEnricher(Enricher):
    """[ALARABIYA] Searches Al Arabiya (via Google News RSS) for mentions of an individual."""
    ...
    @classmethod
    def name(cls) -> str:
        return "individual_to_alarabiya"
    ...
    def preprocess(
        ...
        if not cleaned:
            Logger.error(
                self.sketch_id,
                {"message": "[INDIVIDUAL_TO_ALARABIYA] No valid Individual inputs."},
            )
        return cleaned
    ...
    async def scan(self, data: List[InputType]) -> List[OutputType]:
        ...
        for individual in data:
            ...
            try:
                ...
                Logger.error(
                    self.sketch_id,
                    {"message": f"[ALARABIYA] error for '{query}': {exc}"},
                )
                continue
            for hit in hits:
                website = Website(
...
InputType = IndividualToAlarabiyaEnricher.InputType
OutputType = IndividualToAlarabiyaEnricher.OutputType
