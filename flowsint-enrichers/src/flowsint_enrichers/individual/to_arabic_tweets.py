"""Arabic-language Twitter/X enricher via Nitter: Individual -> Website mentions."""
...
from tools.arabic_media.nitter import NitterArabicTool


REL_LABEL = "MENTIONED_ON_TWITTER_AR"


@flowsint_enricher
class IndividualToArabicTweetsEnricher(Enricher):
    """[NITTER] Searches Arabic tweets (Nitter + Google fallback) for mentions of an individual."""
    ...
    @classmethod
    def name(cls) -> str:
        return "individual_to_arabic_tweets"
    ...
    def preprocess(
        ...
        if not cleaned:
            Logger.error(
                self.sketch_id,
                {
                    "message": "[INDIVIDUAL_TO_ARABIC_TWEETS] No valid Individual inputs."
                },
            )
        return cleaned

    async def scan(self, data: List[InputType]) -> List[OutputType]:
        tool = NitterArabicTool()
        ...
        for individual in data:
            ...
            try:
                ...
                Logger.error(
                    self.sketch_id,
                    {"message": f"[NITTER] error for '{query}': {exc}"},
                )
                continue
            for hit in hits:
                website = Website(
...
InputType = IndividualToArabicTweetsEnricher.InputType
OutputType = IndividualToArabicTweetsEnricher.OutputType
