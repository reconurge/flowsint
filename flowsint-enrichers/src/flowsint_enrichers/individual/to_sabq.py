"""Sabq Arabic news enricher: Individual -> Website mentions."""
...
from tools.arabic_media.sabq import SabqTool


REL_LABEL = "MENTIONED_IN_SABQ"


@flowsint_enricher
class IndividualToSabqEnricher(Enricher):
    """[SABQ] Searches sabq.org Arabic news for mentions of an individual."""
    ...
    def preprocess(
        ...
        if not cleaned:
            Logger.error(
                self.sketch_id,
                {"message": "[INDIVIDUAL_TO_SABQ] No valid Individual inputs."},
            )
        return cleaned
    ...
    async def scan(self, data: List[InputType]) -> List[OutputType]:
        ...
        for individual in data:
            ...
            try:
                ...
                Logger.error(
                    self.sketch_id,
                    {"message": f"[SABQ] error for '{query}': {exc}"},
                )
                continue
            for hit in hits:
                website = Website(
...
InputType = IndividualToSabqEnricher.InputType
OutputType = IndividualToSabqEnricher.OutputType
