"""Al Arabiya news enricher: Phrase (topic) -> Website mentions (Google News RSS)."""
...
@flowsint_enricher
class PhraseToAlarabiyaEnricher(Enricher):
    """[ALARABIYA] Searches Al Arabiya (via Google News RSS) for mentions of a topic/phrase."""
    ...
    def preprocess(
        ...
        if not cleaned:
            Logger.error(
                self.sketch_id,
                {"message": "[PHRASE_TO_ALARABIYA] No valid Phrase inputs."},
            )
        return cleaned
...
InputType = PhraseToAlarabiyaEnricher.InputType
OutputType = PhraseToAlarabiyaEnricher.OutputType
