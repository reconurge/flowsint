"""Arabic-language Twitter/X enricher via Nitter: Phrase (topic) -> Website mentions."""
...
@flowsint_enricher
class PhraseToArabicTweetsEnricher(Enricher):
    """[NITTER] Searches Arabic tweets (Nitter + Google fallback) for mentions of a topic/phrase."""
    ...
    @classmethod
    def name(cls) -> str:
        return "phrase_to_arabic_tweets"
    ...
    def preprocess(
        ...
        if not cleaned:
            Logger.error(
                self.sketch_id,
                {"message": "[PHRASE_TO_ARABIC_TWEETS] No valid Phrase inputs."},
            )
        return cleaned
...
InputType = PhraseToArabicTweetsEnricher.InputType
OutputType = PhraseToArabicTweetsEnricher.OutputType
