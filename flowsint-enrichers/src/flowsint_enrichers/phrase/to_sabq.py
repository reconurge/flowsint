"""Sabq Arabic news enricher: Phrase (topic) -> Website mentions."""
...
@flowsint_enricher
class PhraseToSabqEnricher(Enricher):
    """[SABQ] Searches sabq.org Arabic news for mentions of a topic/phrase."""
    ...
    def preprocess(
        ...
        if not cleaned:
            Logger.error(
                self.sketch_id,
                {"message": "[PHRASE_TO_SABQ] No valid Phrase inputs."},
            )
        return cleaned
...
InputType = PhraseToSabqEnricher.InputType
OutputType = PhraseToSabqEnricher.OutputType
