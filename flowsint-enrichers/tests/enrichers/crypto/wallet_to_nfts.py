from flowsint_enrichers.crypto.wallet_to_nfts import CryptoWalletAddressToNFTs
from flowsint_types.wallet import CryptoWallet, CryptoNFT
from pydantic import HttpUrl

enricher = CryptoWalletAddressToNFTs("sketch_123", "scan_123")


def test_wallet_address_to_transactions_name():
    assert enricher.name() == "wallet_to_nfts"


def test_wallet_address_to_transactions_category():
    assert enricher.category() == "crypto"
...
def test_scan_mocked_transactions(monkeypatch):
    ...
    monkeypatch.setattr(enricher, "_get_nfts", mock_get_nfts)

    input_data = [CryptoWallet(address="0x742d35Cc6634C0532925a3b844Bc454e4438f44e")]
    result = enricher.scan(input_data)
