import pytest
from flowsint_enrichers.crypto.wallet_to_transactions import (
...
def test_wallet_address_to_transactions_name():
    assert enricher.name() == "wallet_to_transactions"


def test_wallet_address_to_transactions_category():
    assert enricher.category() == "CryptoCryptoWallet"
...
async def test_scan_mocked_transactions(monkeypatch):
    ...
    monkeypatch.setattr(enricher, "_get_transactions", mock_get_transactions)

    input_data = [CryptoWallet(address="0x742d35Cc6634C0532925a3b844Bc454e4438f44e")]
    result = await enricher.scan(input_data)
...
def test_enricher_requires_api_key():
    """Test that the enricher validates required ETHERSCAN_API_KEY parameter at construction"""
    with pytest.raises(
        ValueError, match="Enricher wallet_to_transactions received invalid params"
    ):
        CryptoWalletAddressToTransactions("sketch_123", "scan_123", params={})


def test_enricher_with_invalid_api_key_type():
    """Test that the enricher validates parameter types"""
    with pytest.raises(
        ValueError, match="Enricher wallet_to_transactions received invalid params"
    ):
        CryptoWalletAddressToTransactions(
