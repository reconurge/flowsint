...
async def test_scan_returns_ip(monkeypatch):
    ...
    output = await enricher.execute(input_data)
    print(output)
...
class TestResolveInputOutputTypes:
    """Test the InputType/OutputType functionality for ResolveEnricher"""

    def test_input_output_types_are_defined(self):
        """Test that InputType and OutputType are properly defined"""
        assert hasattr(ResolveEnricher, "InputType")
        assert hasattr(ResolveEnricher, "OutputType")
        assert ResolveEnricher.InputType == List[Domain]
        assert ResolveEnricher.OutputType == List[Ip]

    def test_schemas_use_generate_methods(self):
        """Test that schema methods use the new generate methods"""
        # These should work without error
        input_schema = ResolveEnricher.generate_input_schema()
        output_schema = ResolveEnricher.generate_output_schema()
    ...
    def test_schema_methods_return_same_as_generate_methods(self):
        ...
        assert ResolveEnricher.input_schema() == ResolveEnricher.generate_input_schema()
        assert ResolveEnricher.output_schema() == ResolveEnricher.generate_output_schema()

    def test_input_schema_properties(self):
        """Test input schema has expected properties"""
        schema = ResolveEnricher.input_schema()
    ...
    def test_output_schema_properties(self):
        ...
        schema = ResolveEnricher.output_schema()
    ...
    def test_type_accessibility_from_instance(self):
        """Test that types are accessible from enricher instance"""
        enricher_instance = ResolveEnricher("test", "test")

        assert enricher_instance.InputType == List[Domain]
        assert enricher_instance.OutputType == List[Ip]

        # Should be able to generate schemas from instance
        input_schema = enricher_instance.generate_input_schema()
        output_schema = enricher_instance.generate_output_schema()
