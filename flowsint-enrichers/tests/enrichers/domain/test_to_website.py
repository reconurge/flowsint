...
def enricher():
    ...
    return DomainToWebsiteEnricher(sketch_id="test_sketch", scan_id="test_scan")


def test_website_object_serialization(enricher):
...
def test_create_node_with_website(enricher):
...
def test_create_relationship_with_website(enricher):
    ...
    try:
        enricher.create_relationship(domain, website, "HAS_WEBSITE")
        # If we get here without exception, the serialization works
        success = True
    except Exception as e:
        success = False
...
def test_extract_primary_value_with_httpurl(enricher):
    ...
    primary_value = enricher._extract_primary_value(website)
