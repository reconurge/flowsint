...
class MockEnricher(DomainToHistoryEnricher):
    def __init__(self):
...
def test_preprocess_valid_domains(enricher):
    """Test preprocessing with valid domains."""
    domains = [
        Domain(domain="example.com"),
        Domain(domain="example2.com"),
    ]
    result = enricher.preprocess(domains)
...
def test_preprocess_string_domains(enricher):
...
def test_preprocess_dict_domains(enricher):
...
def test_preprocess_invalid_domains(enricher):
...
def test_is_redacted(enricher):
    """Test the __is_redacted method."""
    # Should be redacted
    assert enricher._DomainToHistoryEnricher__is_redacted("REDACTED FOR PRIVACY")
    assert enricher._DomainToHistoryEnricher__is_redacted("redacted for privacy")
    assert enricher._DomainToHistoryEnricher__is_redacted("Some text with PRIVACY in it")

    # Should NOT be redacted
    assert not enricher._DomainToHistoryEnricher__is_redacted("JOHN DOE")
    assert not enricher._DomainToHistoryEnricher__is_redacted("john@doe.com")
    assert not enricher._DomainToHistoryEnricher__is_redacted("123 JOHN STREET")
    assert not enricher._DomainToHistoryEnricher__is_redacted("DOE CITY")


def test_has_non_redacted_info(enricher):
    ...
    assert enricher._DomainToHistoryEnricher__has_non_redacted_info(valid_contact)
    ...
    assert not enricher._DomainToHistoryEnricher__has_non_redacted_info(redacted_contact)

    # Empty contact
    assert not enricher._DomainToHistoryEnricher__has_non_redacted_info({})


def test_extract_individual_from_contact(enricher):
    ...
    individual = enricher._DomainToHistoryEnricher__extract_individual_from_contact(
...
def test_extract_individual_redacted_name(enricher):
...
def test_is_valid_email(enricher):
    """Test the __is_valid_email method."""
    # Valid emails
    assert enricher._DomainToHistoryEnricher__is_valid_email("test@example.com")
    assert enricher._DomainToHistoryEnricher__is_valid_email("user.name@domain.org")
    assert enricher._DomainToHistoryEnricher__is_valid_email("user+tag@example.co.uk")

    # Invalid emails
    assert not enricher._DomainToHistoryEnricher__is_valid_email("invalid-email")
    assert not enricher._DomainToHistoryEnricher__is_valid_email("@example.com")
    assert not enricher._DomainToHistoryEnricher__is_valid_email("test@")
    assert not enricher._DomainToHistoryEnricher__is_valid_email("")


def test_extract_physical_address(enricher):
    ...
    address = enricher._DomainToHistoryEnricher__extract_physical_address(valid_contact)
    ...
    address = enricher._DomainToHistoryEnricher__extract_physical_address(
        redacted_contact
    )
...
@pytest.mark.asyncio
async def test_scan_with_test_data(enricher, test_data, monkeypatch):
    ...
    monkeypatch.setattr(
        enricher, "_DomainToHistoryEnricher__get_infos_from_whoxy", mock_get_infos
    )

    # Test with epios.com domain
    input_domains = [Domain(domain="epios.com")]
    results = await enricher.scan(input_domains)
    ...
    assert len(enricher._extracted_data) == 16

    # Should have extracted individuals
    assert (
        len(enricher._extracted_individuals) > 0
    ...
    for individual_info in enricher._extracted_individuals:
...
def test_postprocess_creates_nodes_and_relationships(enricher, test_data, monkeypatch):
    ...
    try:
        input_domains = [Domain(domain="epios.com")]
        results = loop.run_until_complete(enricher.scan(input_domains))

        # Debug: Check what individuals were extracted
        print(
            f"\n=== DEBUG: _extracted_individuals has {len(enricher._extracted_individuals)} individuals ==="
        )
        for i, individual_info in enumerate(enricher._extracted_individuals):
            individual = individual_info["individual"]
            print(
        ...
        # Now run postprocess
        print(f"\n=== Running postprocess ===")
        enricher.postprocess(results, input_domains)

        # Debug: Check what happened during postprocess
        print(f"=== Postprocess completed ===")
        print(f"Nodes created: {len(enricher.neo4j_conn.nodes_created)}")
        print(f"Relationships created: {len(enricher.neo4j_conn.relationships_created)}")

        # Should have created some nodes
        assert len(enricher.neo4j_conn.nodes_created) > 0

        # Should have created some relationships
        assert len(enricher.neo4j_conn.relationships_created) > 0

        # Check for domain node
        domain_nodes = [
            n for n in enricher.neo4j_conn.nodes_created if n["label"] == "domain"
        ]
        assert len(domain_nodes) > 0

        # Check for individual nodes (should include JOHN DOE)
        individual_nodes = [
            n for n in enricher.neo4j_conn.nodes_created if n["label"] == "individual"
        ]
...
def test_schemas(enricher):
    """Test that the enricher has the expected schemas."""
    input_schema = enricher.input_schema()
    output_schema = enricher.output_schema()
