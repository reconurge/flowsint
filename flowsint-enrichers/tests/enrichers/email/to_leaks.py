...
from flowsint_enrichers.emails.to_leaks import EmailToBreachesEnricher
from flowsint_types.email import Email
from flowsint_types.breach import Breach

enricher = EmailToBreachesEnricher("sketch_123", "scan_123")


def test_enricher_name():
    assert EmailToBreachesEnricher.name() == "to_leaks"


def test_enricher_category():
    assert EmailToBreachesEnricher.category() == "Email"


def test_enricher_key():
    assert EmailToBreachesEnricher.key() == "email"
...
@patch("src.enrichers.emails.to_leaks.requests.get")
...
@patch("src.enrichers.emails.to_leaks.HIBP_API_KEY", None)
def test_scan_no_api_key():
    """Test that enricher raises ValueError when HIBP_API_KEY is not set."""
    emails = [Email(email="test@example.com")]

    with pytest.raises(ValueError, match="HIBP_API_KEY not set"):
        enricher.scan(emails)


def test_postprocess():
    # Test postprocess method with mocked neo4j connection
    enricher.neo4j_conn = MagicMock()
    ...
    result = enricher.postprocess(breaches, original_input)
    ...
    # Total: 5 queries
    assert enricher.neo4j_conn.query.call_count == 5
