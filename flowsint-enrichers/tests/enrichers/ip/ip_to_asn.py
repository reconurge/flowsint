...
from flowsint_enrichers.ips.ip_to_asn import IpToAsnEnricher
...
logger = TestLogger()
# The enricher will get a mock logger from conftest.py automatically
enricher = IpToAsnEnricher("sketch_123", "scan_123", logger)
...
def test_preprocess_valid_ips():
    ...
    result = enricher.preprocess(ips)
...
def test_scan_extracts_asn_info(monkeypatch):
    ...
    # Patch the subprocess call in the enricher
    monkeypatch.setattr("subprocess.run", mock_subprocess_run)

    input_data = [Ip(address="8.8.8.8")]
    asns = enricher.scan(input_data)
...
def test_postprocess_creates_neo4j_relationships(monkeypatch):
    ...
    mock_neo4j = Mock()
    enricher.neo4j_conn = mock_neo4j
    ...
    result = enricher.postprocess(asn_results, input_data)
