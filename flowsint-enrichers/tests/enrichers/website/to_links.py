...
from flowsint_enrichers.websites.to_links import WebsiteToLinks
...
async def test_website_to_links_real_time_neo4j_creation():
    ...
    enricher = WebsiteToLinks(sketch_id="test", scan_id="test")

    # Mock neo4j connection and methods
    enricher.neo4j_conn = Mock()
    enricher.create_node = Mock()
    enricher.create_relationship = Mock()
    enricher.log_graph_message = Mock()

    # Test input
    websites = [Website(url="https://example.com")]

    with patch("src.enrichers.websites.to_links.Crawler", MockCrawler):
        results = await enricher.scan(websites)

    # Verify main website and domain nodes were created upfront
    enricher.create_node.assert_any_call(
        "website",
        "url",
        "https://example.com",
    ...
    enricher.create_relationship.assert_any_call(
        "website",
        "url",
        "https://example.com",
        "domain",
...
async def test_website_to_links_error_handling_with_neo4j():
    ...
    with patch("src.enrichers.websites.to_links.Crawler", mock_crawler_error):
        results = await enricher.scan(websites)
...
def test_postprocess_simplified():
    ...
    processed_results = enricher.postprocess(results, original_input)
