...
export async function GET(_: Request, { params }: { params: Promise<{ investigation_id: string }> }) {
    try {
        ...
        const { data: sketches, error } = await supabase.from("sketches")
            .select("id, title, description, status, owner_id, investigation_id, last_updated_at, investigation:investigations(id, name), members:sketches_profiles(profile:profiles(id, first_name, last_name, avatar_url), role)")
