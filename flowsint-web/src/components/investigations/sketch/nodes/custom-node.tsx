...
import { cn, typeColorMap, zoomSelector } from '@/lib/utils';
...
export default memo(({ data, selected }: any) => {
    ...
    return (
        <TooltipNode className={cn('p-.5 rounded-full', typeColorMap[data.type])} selected={selected}>
            <TooltipTrigger>
                <div className="inline-flex items-center gap-2 p-1 w-auto">
                    ...
                    {settings.showNodeLabel && <div className="flex items-center gap-1">
            ...
            {!settings.showNodeLabel &&
                <TooltipContent position={Position.Top}>{data.label}</TooltipContent>
            }
            <Handle
                type="target"
